class EitIcons extends HTMLElement {
  ...
  async render() {
    ...
    if (withCircle) {
      ...
      svgContent = svgContent.replace('<g class="eit-icon">', `<g class="eit-icon" style="color: white;">`);

      // Inject the style into the SVG
      const styleStart = '<style>';
      const styleEnd = '</style>';
      svgContent = svgContent.replace('<svg', `<svg><defs>${styleStart}${circleStyle}${styleEnd}</defs>`);
      ...
      `;
       ...
       const styleStart = '<style>';
       const styleEnd = '</style>';
       svgContent = svgContent.replace('<svg', `<svg><defs>${styleStart}${scalingStyle}${styleEnd}</defs>`);
    }
